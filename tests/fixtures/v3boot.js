...
export async function bootV3(page, routes = {}, { features: overrides = null } = {}) {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (key) {
      ...
      // optionally held back by __delayMs like the JSON form below.
      if (body && typeof body === "object" && body.__fulfill) {
        if (body.__delayMs) await new Promise((resolve) => setTimeout(resolve, body.__delayMs));
        return route.fulfill(body.__fulfill);
      }
