...
- **V3 is not standalone**: its import closure pulls 52 files from `src/js/`, plus
...
- `server.js` mounts `server/routes/*.js` (32 route modules) under `/api`. Route order
...
- Flags live in `src/js/config.js` under `features:` (100 flags), copied to
...
- `npm test` runs the Playwright suite — 133 spec files in `tests/` (plus 2 in `tests/verify/`,
