...
export class CalendarService {
  ...
  async getMergedEvents() { const urls = getConfiguredCalendarUrls(); const results = await Promise.all(urls.map(({ name, url }) => this.fetchCalendar(url, name))); const merged = results.flat().sort((a, b) => Date.parse(a.start) - Date.parse(b.start)); this.eventBus.emit({ type: 'CALENDAR_UPDATED', payload: { events: merged, count: merged.length } }); return merged; }
}
